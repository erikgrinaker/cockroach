...
// appropriate, e.g. return it up the stack, ignore it, or fatal if necessary.
//
// Additionally, fatal assertions can be enabled/disabled via the environment
// variable COCKROACH_FATAL_ASSERTIONS, regardless of build type.
//
...
var (
	...
	// assertion error is returned and propagated.
...
// stack trace, and reports it to Sentry (if enabled). ctx can be nil.
...
// stack trace and the source code origin of the log message. ctx can be nil.
...
// in release builds or when fatal assertions are disabled via
// COCKROACH_FATAL_ASSERTIONS. This should be used very rarely, only when it is
// unsafe to keep the process running.
//
// gcassert:inline
...
// builds. The given function should use assertions as usual and return
// their result.
...
// EqualBytes requires two byte slices or strings to be equal. Otherwise,
// fatals in dev/test builds, or returns an assertion error that's also logged
// and reported in release builds.
//
// gcassert:inline
